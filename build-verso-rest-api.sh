...
##############################################################################
# Script de Construction - verso-rest-api Plugin
# Crée le plugin verso-rest-api prêt au déploiement
#
# Usage: ./build-verso-rest-api.sh
##############################################################################

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BUILD_DIR="$SCRIPT_DIR/verso-rest-api"

echo "🔨 Construction du plugin verso-rest-api..."
echo ""

# Créer le répertoire du plugin
mkdir -p "$BUILD_DIR"

echo "[1/3] Création du fichier du plugin..."

cat > "$BUILD_DIR/verso-rest-api.php" << 'EOFPHP'
<?php
/**
...
echo "✓ verso-rest-api.php créé ($(stat -f%z "$BUILD_DIR/verso-rest-api.php" 2>/dev/null || stat -c%s "$BUILD_DIR/verso-rest-api.php") bytes)"

echo "[2/3] Création du README..."
...
if [ -f "$BUILD_DIR/verso-rest-api.php" ] && [ -f "$BUILD_DIR/README.md" ]; then
    echo "✓ Plugin prêt au déploiement"
else
...
echo ""
echo "✅ Plugin verso-rest-api créé!"
echo ""
echo "📁 Fichiers:"
ls -lh "$BUILD_DIR/"
echo ""
